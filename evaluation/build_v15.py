"""Build v15 layout: join FRESH, content-hash-deduped ImageNet real photos (shards
00023-00025, never used) to the real class, on top of the v14 corpus. The dedup
removes any exact-duplicate of the truly-unseen oracle (data_unseen/real_eval,
1500 never-trained) so the oracle stays clean.

Layout:
  data_v15/train/fake        <- reuse v14 AI
  data_v15/train/real        <- v14 real + fresh deduped ImageNet photos (REP)
  data_v15/test/fake/*       <- per-generator held-out (reuse)
  data_v15/test/real         <- training-family held-out (reuse)
  data_v15/test/real_unseen  <- the SAME 1500 oracle, NEVER trained
"""
import os, sys, glob, hashlib

FRESH = sys.argv[1]       # data_v15_more (sh* subdirs of fresh ImageNet)
...
for f in sorted(fres):
    ...
    if h in oracle_hashes:
        n_dup += 1
        continue
    shard_key = os.path.basename(os.path.dirname(f))  # e.g. sh00023
    os.symlink(os.path.abspath(f), os.path.join(f"{OUT}/train/real", f"v15_{shard_key}_{os.path.basename(f)}"))
    n_add += 1
    if n_add >= REP:
        break
print(f"+{n_add} fresh real added; {n_dup} dup-oracle excluded", flush=True)

# fresh unseen eval slice (oracle, untouched) — identical to v14
